...
async def upload_to_gdrive(file_upload, message, messa_ge, g_id):
    ...
    if os.path.isfile(file_upload):
        ...
        await asyncio.sleep(EDIT_SLEEP_TIME_OUT)
        await messa_ge.reply_text(f"🤖: {file_upload} <b>has been Uploaded successfully to your Cloud😌</b> <i><b>💥Join Our TEAM DRIVE to Access Link😊You can't Access Link without Join Team Drive👉 /JOIN_TEAM_DRIVE</b></i>  |<a href='tg://user?id={g_id}'> </a>", reply_markup=button_markup),
